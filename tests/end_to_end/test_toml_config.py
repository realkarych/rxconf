...
def test_empty() -> None:
    conf = Conf.from_file(config_path=_RESOURCE_DIR / "empty.toml")
...
async def test_empty_async() -> None:
    conf = await AsyncConf.from_file(config_path=_RESOURCE_DIR / "empty.toml")

    assert conf._MetaTree__config._root == {}


def test_wrong_equality() -> None:
    conf = Conf.from_file(config_path=_RESOURCE_DIR / "empty.toml")._MetaTree__config
    with pytest.raises(exceptions.RxConfError):
        assert conf == 1


def test_correct_equality() -> None:
    conf = Conf.from_file(config_path=_RESOURCE_DIR / "primitives.toml")
    another_conf = Conf.from_file(config_path=_RESOURCE_DIR / "primitives.toml")
...
async def test_primitive_types_async() -> None:
    conf = await AsyncConf.from_file(config_path=_RESOURCE_DIR / "primitives.toml")
...
def test_inner_structures() -> None:
    conf = Conf.from_file(config_path=_RESOURCE_DIR / "inner_structures.toml")
