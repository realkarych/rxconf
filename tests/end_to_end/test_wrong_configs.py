...
async def test_not_existing_yaml_async() -> None:
    with pytest.raises(exceptions.ConfigNotFoundError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "something_not_exist.yaml")
...
async def test_broken_schema_yaml_async() -> None:
    with pytest.raises(exceptions.BrokenConfigSchemaError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "broken_schema.yml")
...
async def test_not_existing_json_async() -> None:
    with pytest.raises(exceptions.ConfigNotFoundError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "something_not_exist.json")
...
async def test_broken_schema_json_async() -> None:
    with pytest.raises(exceptions.BrokenConfigSchemaError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "broken_schema.json")
...
async def test_not_existing_toml_async() -> None:
    with pytest.raises(exceptions.ConfigNotFoundError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "something_not_exist.toml")
...
async def test_broken_schema_toml_async() -> None:
    with pytest.raises(exceptions.BrokenConfigSchemaError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "broken_schema.toml")
...
async def test_unrecognized_extension_async() -> None:
    with pytest.raises(exceptions.InvalidExtensionError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "something.tpl")
...
async def test_not_existing_ini_async() -> None:
    with pytest.raises(exceptions.ConfigNotFoundError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "something_not_exist.ini")
...
async def test_broken_schema_ini_async() -> None:
    with pytest.raises(exceptions.BrokenConfigSchemaError):
        assert await AsyncRxConf.from_file(config_path=_RESOURCE_DIR / "broken_schema.ini")
