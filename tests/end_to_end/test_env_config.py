...
def test_primitive_types() -> None:
    conf = Conf.from_file(config_path=_RESOURCE_DIR / "primitives.env")
...
def test_correct_equality() -> None:
    conf = Conf.from_env()
    another_conf = Conf.from_env()
    file_conf = Conf.from_file(config_path=_RESOURCE_DIR / "primitives.env")
    another_file_conf = Conf.from_file(config_path=_RESOURCE_DIR / "primitives.env")
...
def test_env_prefix() -> None:
    conf1 = Conf.from_env(prefix="some_prefix_")
    assert conf1.some_prefix_value1 == 1
    assert conf1.some_PREFIX_value2 == 2

    conf2 = Conf.from_env(prefix="some_prefix", remove_prefix=True)
    assert conf2.value1 == 1
    assert conf2.value2 == 2

    conf3 = Conf.from_env(prefix="somePrefix", remove_prefix=True)
...
async def test_from_env_async() -> None:
    conf2 = await AsyncConf.from_env(prefix="some_prefix", remove_prefix=True)
    assert conf2.value1 == 1
    assert conf2.value2 == 2
...
async def test_primitive_types_async() -> None:
    conf = await AsyncConf.from_file(config_path=_RESOURCE_DIR / "primitives.env")
