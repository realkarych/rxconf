...
from rxconf import exceptions, config_types, config_resolver
...
def test_resolve_toml_file(resolver):
    path = "config.toml"
    with patch("os.path.splitext", return_value=("config", ".toml")), \
...
def test_resolve_ini_file(resolver):
    path = "yaml.toml.ini"
    with patch("os.path.splitext", return_value=("yaml.toml.ini", ".ini")), \
...
def test_resolve_dotenv_file(resolver):
    path = ".env"
    with patch("os.path.splitext", return_value=(".env", ".env")), \
